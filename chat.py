...
class ChatRoomHandler(BaseHandler):
    ...
    def get(self):
        self.render("chatroom.html", messages=MessageMixin.catchs,
                    clients=MessageMixin.clients)
...
class MessageMixin(object):
    ...
    def add_client(self, client):
        ...
        cls.catchs.append({'name': client.name, 'message': 'has joined',
                          'time': now, 'id': id(client)})
        for client in cls.clients:
            client.send(tornado.escape.json_encode({'type': 1, 'id': id(client),
                      'time': now, 'user': client.name, 'message': 'has joined'}))
    ...
    def remove_client(self, client):
        ...
        cls.catchs.append({'name': client.name, 'message': 'has joined',
                          'time': now, 'id': id(client)})
        for client in cls.clients:
            client.send(json.dumps({'type': 2, 'id': id(client), 'time': now,
                       'user': client.name, 'message': 'has left'}))
    ...
    def new_message(self, client, message):
        ...
        if self.is_spam(message, self._get_last_message(c_id)):
            cls = MessageMixin
            now = time.strftime('%Y-%m-%d %H:%M:%S')
            cls.catchs.append({'name': client.name, 'message': message,
                              'time': now, 'id': id(client)})
            for client in cls.clients:
                client.send(json.dumps({'type': 3, 'time': now, 'user': name,
                            'message': message}))
        else:
